...
trackbox.api.theme.shell.render = function () {
	...
	getFileContents(trackbox.api.theme.shell.current.shell, function (shell) {
		$("body").html(shell);
		trackbox.api.theme.page.load();
		trackbox.api.localization.load();
		trackbox.api.router.route();
		trackbox.api.isLoaded = true;
	});
};
